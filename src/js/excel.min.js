...
function extractPeriod(sheet) {
  const matches = ((sheet.A4 || {}).v || '').match(/Del: (.*?) Al: (.*?)$/);
...
function extractMainData(sheet) {
  const data = XLSX.utils.sheet_to_json(sheet, { header: 1 }),
    notesStartIndex = data.findIndex(
...
function extractTotals(sheet) {
  const data = XLSX.utils.sheet_to_json(sheet, { header: 1 }),
    lastDataIndex = data.findIndex(
      (row, index) => index > 8 && (!row[1] || '' === row[1])
    ),
    totalsRow = data[lastDataIndex];
  return totalsRow
    ? {
        unidades: {
...
function displayTable(data) {
  const tableHead = document.querySelector('#excelTable thead'),
    tableBody = document.querySelector('#excelTable tbody'),
    periodText = `${data.period.start} al ${data.period.end}`,
  ...
  [
    ...
    const th = document.createElement('th');
    (th.textContent = text), subHeaderRow.appendChild(th);
  }),
    ...
    data.data.mainData.forEach((row) => {
      ...
      [
        ...
        if (11 === index && cell) {
          ...
          note && (td.classList.add('has-tooltip'), (td.title = note));
        } else td.textContent = null != cell ? cell : '0';
        index >= 3 &&
          index <= 10 &&
  ...
  [
    ...
    const td = document.createElement('td');
    (td.textContent = null != cell ? cell : '0'),
      index >= 3 &&
        index <= 10 &&
        ((td.textContent = formatNumber(Number(cell))),
        td.classList.add('number-cell')),
